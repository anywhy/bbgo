...
func TestNonce(t *testing.T) {
	...
	// Test GetInt64
	nonce1 := nonce.GetInt64()
	nonce2 := nonce.GetInt64()

	if nonce1 >= nonce2 {
		t.Error("Nonce should be incrementing")
	}

	// Test GetString
	nonceStr := nonce.GetString()
	if nonceStr == "" {
		t.Error("Nonce string should not be empty")
	}

	// Verify string is numeric
	_, err := strconv.ParseInt(nonceStr, 10, 64)
	if err != nil {
		t.Errorf("Nonce string should be numeric: %v", err)
	}
}
